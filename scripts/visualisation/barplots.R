...
BaRploTs <- function(metric){
  ...
  barplt <- ggplot(data = final_df, aes(fill = CAT, x = TIME, y = COUNTS)) +
    ...
    annotate(geom = "text", x = 205, y = 24000, label = "Seton Time Limit", angle = 90, size = 3, fontface = "italic", col = "red") +
    annotate(geom = "text", x = 415, y = 24000, label = "Matthews Time Limit", angle = 90, size = 3, fontface = "italic", col = "red")
