...
df <- data.frame(spdp) #dataframe of the coordinates of the centroids
df$Beginning <- 540
df$End <- 0
df <- SpatialPointsDataFrame(coords = df[,1:2], data = df)
proj4string(df)<- CRS("+proj=longlat +datum=WGS84") #assign coord system to the SpatialPointsDataFrame
...
for(mdl in models){
  ...
  georef <- sp::over(df, cont_pol)$PLATEID1 #we overlay our spatial df with the continental polygons by plate ID
...
df <- df[-index_to_drop, ] #finally get rid of them

## export the spatial points dataframe as a shapefile --------------------------
raster::shapefile(df, "./data/meshgrid/meshgrid.shp", overwrite = TRUE)  #create shapefile from the SpatialPtsDF and save it with the same command
